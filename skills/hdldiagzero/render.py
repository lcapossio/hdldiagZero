...
def render(spec_path, out_path, theme_override=None):
    ...
    if spec.get("title"):
        out.append(f'  <text x="{canvas_w/2:.1f}" y="24" text-anchor="middle" '
                   f'font-size="17" font-weight="600" fill="{theme["ink"]}" '
    ...
    if domains:
        out.append(f'  <text x="{g["margin"]}" y="{sy_legend:.0f}" '
                   f'font-size="11" font-weight="600" fill="{theme["ink_soft"]}" '
        ...
        for name, info in domains.items():
            ...
            out.append(f'  <text x="{sx + 24}" y="{sy + 11}" font-size="11" '
    ...
    if used_kinds:
        ...
        for k in used_kinds:
            ...
            out.append(f'  <text x="{text_x}" y="{sy + 11}" font-size="11" '
