...
# Per-glyph advance widths (1/1000 em) for the sans stack above, from the Adobe
# Helvetica AFM metrics (Arial is metrically near-identical). Edge-label width
# drives collision-aware label placement, so summing real advances beats a flat
# monospace ratio - a fixed factor over-reserves space for narrow labels and
# under-reserves for wide ones. validate.py carries the same table so its text
# checks measure labels exactly as they were placed.
GLYPH_ADVANCE = {
